...
- **dining_pairs** — ผลจับคู่บิล↔สลิป (group_id, pair_date, table_no, bill_amount, slip_amount, **diff** [สลิป−บิล, ลบ=ขาด], recorded_at, source ['รูปเดียว'/'จับคู่เวลา']). ระบบ D
...
- กระทบบิล-สลิป (เฉพาะ DINING_GROUPS): ส่งรูปบิลร้าน+สลิป (AI แยกเอง) • `ยอดสะสม`/`ยอดทริป` (ดูยอด+รายการโอนขาด + **การ์ดปุ่มยืนยันเก็บครบ**) • `เก็บครบแล้ว`/`ปิดยอด` หรือ **กดปุ่ม ✅ เก็บครบแล้ว** (postback `dining_confirm`, กันกดซ้ำด้วย `_postback_once`) → รีเซ็ตยอดทริปเป็น 0
...
- `DINING_SHORT_BAHT`(1) — เตือนเมื่อโอน 'ขาด' ตั้งแต่กี่บาทขึ้นไป (≥); `DINING_MATCH_MIN`(45) — บิลค้างรอจับคู่สลิปได้นานกี่นาที (เกิน=หมดอายุ)
