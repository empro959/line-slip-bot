...
- **B. จองโต๊ะ** — AI จับข้อความจอง → การ์ดปุ่มคอนเฟิร์ม, จองล่วงหน้าส่งกลุ่มบาร์น้ำ; เตือนซ้ำ+สรุปรอบเช้า (ดีฟอลต์ 11:00, ตั้งด้วย `RESV_SUMMARY_HOUR`) = 'เฉพาะจองวันนี้' (จองล่วงหน้าแจ้งเฉพาะวันงาน; ดูล่วงหน้าได้ที่คำสั่ง 'สรุปจอง')
...
- `maybe_send_resv_summary` — กรอบ `RESV_SUMMARY_HOUR`:00 ถึง +3 ชม. วันละครั้ง (ดีฟอลต์ 11:00–13:59): สรุปจอง 'เฉพาะวันนี้' (จองล่วงหน้าโผล่เมื่อ resv_date=วันนี้) → BAR_GROUP + RESV_GROUPS; ไม่มีจอง=ไม่ส่ง
...
- `_reservation_reminder_loop` — เตือนจอง PENDING (จองสดวันนี้) ทุก `RESV_NAG_INTERVAL_MIN` นาที (ดีฟอลต์ 30, ทุกช่วงเวลาเท่ากัน) จนคอนเฟิร์ม/เกิน `RESV_NAG_MAX_HOURS` (ดีฟอลต์ 3 ชม.)
...
- `RESV_NAG_MAX_HOURS`(3), `RESV_NAG_INTERVAL_MIN`(30 = ตื๊อคอนเฟิร์มทุกกี่นาที), `RESV_SUMMARY_HOUR`(11 = เวลาส่งสรุปจองวันนี้อัตโนมัติ), `RESV_REPORT_DAYS`(7), `RESV_KEEP_DAYS`(15), `MISS_KEEP_DAYS`(14), `SLIP_KEEP_DAYS`(60), `SLIP_WORKERS`(2)
