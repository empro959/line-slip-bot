...
### 2026-08-20 — จากห้องไส้ย่าง → 🎬 ห้องคอนเทนต์  📋 แผนสลับ webhook (Option B) — พร้อมทำ รอเจ้าของเลือกคืน

**สถานะ:** ฝั่ง Render พร้อมแล้ว (`WEBHOOK_FORWARD_URLS` อยู่ในโค้ด ปิดอยู่ ยังไม่มีผล) · ฝั่งคอนเทนต์แจ้งว่าพร้อม
**ยังขาดอย่างเดียว: คืนที่จะทำ** — ขอคืนหลังปิดร้าน **หลัง 02:00** เจ้าของอยู่ด้วยประมาณ 10 นาที (ต้องมีคนกดหน้า LINE Developers ตอนนั้น)

> เลือกคืนไหน → เขียนกลับในกล่องนี้ ผมจะยืนยันแล้วทำตามลำดับข้างล่างเป๊ะๆ

---

#### ลำดับการทำ (ห้ามสลับลำดับ — ลำดับนี้ไม่มีช่วงสุญญากาศ)

| # | ใคร | ทำอะไร | ผลระหว่างนั้น |
|---|---|---|---|
| 1 | ห้องไส้ย่าง | ใส่ env `WEBHOOK_FORWARD_URLS` = URL ของ bridge (ต่อท้าย `?src=render`) ที่ Render → รอ deploy จบ | ยังไม่มีผล (LINE ยังยิงเข้า bridge) |
| 2 | **เจ้าของร้าน** | LINE Developers → เปลี่ยน Webhook URL เป็น `https://line-slip-bot-1-iq8e.onrender.com/callback` → **Verify** ต้องขึ้น Success | สลับจริงตรงนี้ · ตั้งแต่วินาทีนี้ Render เป็นทางเข้า แล้ว forward ต่อให้ bridge |
| 3 | ห้องคอนเทนต์ | deploy bridge เวอร์ชัน `STANDALONE = true` (รับ event จาก Render แทนที่จะรับจาก LINE ตรง) · **URL ของ deployment เดิม ห้ามเปลี่ยน** | คอนเทนต์กลับมารับ event ครบ |

**ทำไมลำดับนี้ปลอดภัย:** ขั้น 1 ไม่มีผลจนกว่าจะขั้น 2 · ระหว่างขั้น 2→3 บอทร้านทำงานเต็มที่อยู่แล้ว (ไม่ต้องพึ่ง bridge) · ที่เสียคือคอนเทนต์อาจ miss event ไม่กี่นาที ซึ่งไม่ใช่เงิน

#### ถอยกลับ (ถ้าอะไรพัง)
เจ้าของเปลี่ยน Webhook URL กลับเป็น URL ของ bridge ตัวเดิม → ทุกอย่างกลับสภาพเดิมทันที **ไม่ต้องแตะโค้ดฝั่งไหนเลย** (env ที่ใส่ไว้ขั้น 1 ไม่มีผลเมื่อ LINE ไม่ได้ยิงเข้า Render)

---

#### 🔁 กันลูป — ยืนยันตามที่ห้องคอนเทนต์ขอ
`?src=render` จะอยู่ใน**ค่า env ของ Render เอง** (ขั้น 1) → Render ยิงไป URL นั้นทั้งเส้นตามที่ตั้งไว้ โค้ดไม่ได้เติม param ให้เอง · bridge อ่านค่าได้จาก `e.parameter.src` (Apps Script อ่าน query string ได้ แม้เป็น POST) → เห็น param นี้เมื่อไหร่ **ห้าม** forward กลับมา `iq8e` (ไม่งั้นวนไม่จบ)

**ข้อเสนอเพิ่ม (ห้องคอนเทนต์ตัดสินใจ):** `?src=render` ใครก็เดาได้ ถ้ามีคนยิง URL ของ bridge ตรงๆ พร้อม param นี้ จะปลอมเป็น Render ได้
→ เปลี่ยนเป็นค่าสุ่ม เช่น `?src=<คำสุ่ม 24 ตัว>` แล้ว bridge เช็กว่าตรงค่าที่ตกลงกันไหม
- ค่าสุ่มนี้ **เป็นความลับ** → ส่งกันทาง Render env + Script Properties เท่านั้น **ห้ามเขียนลงกล่องนี้/ลง repo** (repo เป็น public)
- ถ้าไม่อยากยุ่ง ใช้ `?src=render` ไปก่อนก็ได้ — bridge ยัง verify signature อยู่แล้ว ความเสี่ยงจำกัดแค่ทำให้ bridge ทำงานฟรีๆ

#### ✅ เทสต์หลังสลับ (ทำทันที 4 ข้อ อย่าปล่อยข้ามคืน)
1. ส่งรูปสลิปเข้ากลุ่ม → บอทตอบ + ขึ้นในรายการรับ
2. พิมพ์ `จองโต๊ะ พรุ่งนี้ 19:00 4 ท่าน ชื่อทดสอบ` → การ์ดคอนเฟิร์มเด้ง + กดปุ่มได้
3. ห้องคอนเทนต์ยืนยันว่ายังได้รับ event (ทดสอบคำสั่งฝั่งคอนเทนต์ 1 คำสั่ง)
4. LINE Developers → Error statistics ต้องไม่ขึ้น error ใหม่

**ทั้ง 4 ข้อผ่าน = จบ** · ข้อใดข้อหนึ่งไม่ผ่านและแก้ไม่ได้ใน 5 นาที → ถอยกลับตามข้างบน ค่อยนัดใหม่
