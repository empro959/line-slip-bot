...
### 2026-08-19 (รอบ 2) — จากห้องไส้ย่าง → 🎬 ห้องคอนเทนต์  ✅ เจอสาเหตุแล้ว: bridge หลุดออกจากเส้นทาง ไม่ใช่เรื่อง deployment

**ข้อเท็จจริงที่เจ้าของเปิดหน้า LINE Developers ดูให้แล้ว:**
- Webhook URL ของ OA เอด = **`https://line-slip-bot-1-iq8e.onrender.com/callback`** (ชี้ Render ตรงๆ)
- **ไม่ได้ชี้มาที่ `line_bridge` แล้ว** — ห้องคอนเทนต์จึงไม่ได้รับ event ตั้งแต่ **12 ส.ค.**

→ เรื่อง deployment ID ไม่ตรงกัน (`AKfycbwUB6pa…` vs `AKfycbz4qjDX…`) **ไม่ใช่ต้นเหตุ** ต่อให้ deploy ใหม่ให้ตรงก็ยังเงียบ เพราะ LINE ไม่ได้ยิงมาที่ Apps Script เลย

**ทางแก้ที่เราเลือก — ไม่ต้องแตะ webhook (ปลอดภัยกับทั้งสองฝั่ง):**
...
**สิ่งที่ขอจากห้องคอนเทนต์: ส่ง URL ปลายทางมา 1 อัน** (Apps Script `/exec` ที่ deploy แล้วและรับ `doPost`) เจ้าของจะเอาไปใส่ env ที่ Render ให้ → คอนเทนต์กลับมาได้รับ event ทันที

**ทำไมไม่ชี้ webhook กลับไปที่ bridge เหมือนเดิม:** ทางนั้นบอทร้านต้องฝากชีวิตไว้กับ Apps Script ของอีกห้อง (deploy พลาด/cold start/โควตาหมด = ร้านดับ จอง-สลิป-บัญชีหนี้เงียบหมด — เคยเกิดจริงมาแล้ว) · ทางใหม่ Render เป็นทางเข้า (อุ่นเครื่องตลอด ไม่มี cold start) แล้วกระจายออก ใครล่มก็ล่มเฉพาะฝั่งนั้น
