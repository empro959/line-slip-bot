...
class TestLedgerPromptHardened(unittest.TestCase):
    ...
    def test_ห้ามรัดจนตัดสมุดจดจริงทิ้ง(self):
        """เคสจริง 15 ก.ย. 23:1x: เงื่อนไข "ต้องเห็นคอลัมน์ รับ สด/โอน จริงๆ" คัดสมุดจดแท้ออก 3 ใบติด
        (ถ่ายเอียง/ครอปหัวตาราง) → ต้องยอมรับว่าหัวคอลัมน์อาจมองไม่เห็น
        📌 พรอมป์ตคือการขอร้อง ไม่ใช่การรับประกัน — ให้ตาข่ายในโค้ด (ทุกช่อง 0) คัดปลายทางแทน"""
        p = self._prompt()
        self.assertNotIn("'สด' กับ 'โอน' จริงๆ", p, "เงื่อนไขนี้แน่นเกินไป ตัดของจริงทิ้ง")
        self.assertIn("ถูกครอป", p, "ต้องบอกว่าหัวคอลัมน์อาจมองไม่เห็นได้")
