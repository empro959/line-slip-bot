...
def extract_slip_info(image_bytes: bytes, retry: bool = False) -> dict:
    ...
    prompt = (
        ...
        "(รวมสลิปโอนทุกธนาคาร, เติมเงินพร้อมเพย์/วอลเล็ต/ทรูมันนี่, เป๋าตัง/รัฐช่วยจ่ายที่ขึ้น 'สำเร็จ')\n"
        "⚠️ ตีความกว้างไว้ก่อน เพื่อไม่ให้สลิปจริงหลุด: ถ้ารูปมีลักษณะเป็นหลักฐานการเงิน "
        "(มีจำนวนเงิน + ข้อความ/เครื่องหมายว่าสำเร็จ หรือมีวันเวลา/เลขอ้างอิง) "
        "แม้ภาพไม่ชัด เอียง ถ่ายจากหน้าจอ มีแสงสะท้อน เบลอ หรือเห็นเพียงบางส่วน "
        "ก็ให้ is_slip=true ไว้ก่อน แล้วพยายามอ่านค่าให้ได้มากที่สุด\n"
        "ใส่ false เฉพาะรูปที่ชัดเจนว่าไม่ใช่หลักฐานการเงินจริงๆ (รูปคน อาหาร เมนู วิว สติกเกอร์ ภาพทั่วไป)\n"
        "📌 สำคัญ: ถ้าในรูปมี 'ทั้งใบเสร็จ/ใบแจ้งรายการของร้าน และ สลิปโอน/หลักฐานจ่ายเงิน' อยู่คู่กัน "
        "(เช่น ถ่ายใบบิลกระดาษวางคู่กับมือถือที่โชว์สลิปโอน) → is_slip=true เสมอ "
        "และให้อ่าน amount/sender/receiver/receiver_account/bank/ref_number/datetime จาก 'สลิปโอน' เท่านั้น "
        "ห้ามอ่านจากใบเสร็จ/ใบแจ้งรายการของร้าน (ยอดบนบิลร้านอาจไม่เท่ายอดที่โอนจริง)\n"
...
def extract_payable_doc(image_bytes: bytes) -> dict:
    ...
    prompt = (
        ...
        "  • ถ้าเป็น 'bill': ใช้ 'ยอดรวมค่าสินค้า' = ผลรวมราคาของทุกแถวสินค้า (จำนวน × ราคาต่อหน่วย)\n"
