...
## 2026-08-18 — จากห้องบอท → ทุกห้อง  ✅ ปิดงานค้างครบ + ของที่ต้องรู้

**ปิดครบแล้ว:** UptimeRobot ชี้ URL ใหม่ (รายงาน 00:30 กลับมาแล้ว ยืนยันจากของจริง) ·
export Supabase เก่าเก็บไว้แล้ว · บัญชีหนี้ล้าง+วางยอดค้างใหม่ ยอดถูกต้องแล้ว ·
Apps Script อัปเดตแล้ว · ไล่ช่องว่างข้อมูลจนจบ **ไม่มีวันไหนหายจริง**

**⚠️ ของที่ต้องรู้ก่อนไปไล่ "ข้อมูลหาย":** ดูหัวข้อ **5.1 ใน `HANDOFF.md`** ก่อนเสมอ —
รวม 4 กรณีที่ "ขาดแต่ปกติ" ไว้แล้ว (ร้านหยุด 29-30 ก.ค. · พ.ค. 2569 ยังไม่เริ่มทำระบบ ·
เดือนเก่าไม่มี `daily_totals` · "วันนี้" ยังไม่มีข้อมูลเป็นปกติ)
ก่อนหน้านี้ข้อมูลพวกนี้อยู่ในหัวเจ้าของคนเดียว ทำให้ 29 ก.ค. หลุดไปอยู่ในลิสต์งานค้าง
ให้คนไล่กู้เปล่าๆ มาแล้วรอบหนึ่ง

**บทเรียนซ้ำของวันนี้ — "เงียบ" อันตรายกว่า "พัง":** วันนี้เจอ 3 จุดที่ปัญหาถูกกลบเงียบ
คือ UptimeRobot แดงแต่ไม่มีใครรู้ (monitor โปรเจกต์อื่นเขียวบังอยู่) · `/api/db_export`
ข้ามตารางที่ไม่อยู่ในลิสต์ hardcode · `catch(e){}` ว่างเปล่าตอนอ่านไฟล์แยกตามลูกค้า
ทำให้ "บิล 0" แยกไม่ออกว่าไม่มีจริงหรืออ่านไม่ได้ — **แก้ทั้ง 3 จุดให้ส่งเสียงแล้ว**
เขียนโค้ดใหม่ในโปรเจกต์นี้ ขอให้ยึดกติกาเดิม: ต้องมีทางให้รู้ว่าพัง
...
**🧪 มีชุดเทสต์ในรีโปแล้ว (`tests/` — 18 เคส, commit `51da09e`):**
```bash
...
→ **แก้ logic การเงินเมื่อไร รันก่อน push เสมอ** · commit นี้ไม่แตะ `app.py`

**หมายเหตุ:** บัญชี GitHub (`empro959`) กับบัญชีบริการอื่น (Render/Supabase/Apps Script/Netlify = `saiyangsoi@gmail.com`) **คนละตัวกัน ตั้งใจให้เป็นแบบนี้** ไม่ใช่ของค้างที่ลืมย้าย

---

## 2026-08-17 — จากห้องบอท → ห้อง dashboard  🔴 กับดักหลังย้ายเซิร์ฟเวอร์ + แก้บั๊กเงียบ 3 ตัว

**บริบท:** หลังย้าย Render/Supabase มาบัญชีร้าน (URL บอทเปลี่ยนเป็น `line-slip-bot-1-iq8e.onrender.com`) มีของที่ยัง "จำ URL เก่า" ค้างอยู่หลายที่ ทำให้ระบบพังเงียบเป็นสัปดาห์โดยไม่มีสัญญาณ

**⚠️ ห้อง dashboard ต้องรู้ (แก้แล้ววันนี้):**
- Script Property **`SLIP_API_URL`** ยังชี้บอทตัวเก่า `65gt` → `pushOwner_` ใช้ host เดียวกันนั้น **สรุปรายรับจึงไม่เข้า LINE หลายวัน** (แต่อีเมลยังมา เพราะ `sendDailyAlert` ทำ LINE แบบ best-effort แล้ว `catch(e){}` กลืน error ทิ้ง) และ `fetchSlipMap_()` ก็พังไปด้วย → ช่องเทียบสลิปใน dashboard เป็น 0 ทั้งเดือน
- **`dashboard/Code_full.gs` ในคอมมิตนี้ = สำเนาที่ตรงกับของจริงบน Apps Script แล้ว** (ของเดิมเป็น snapshot 13 ส.ค. เก่ากว่าที่ deploy อยู่) เพิ่ม 3 ฟังก์ชัน: `showSlipUrl()` / `fixSlipUrl()` (ดู-แก้ host โดยคง token) และ **`importPosByDate()`**
- **`pushOwner_` คืนข้อความ error แล้ว** (ไม่กลืนเงียบ) และถ้า LINE ส่งไม่ออก **หัวเรื่องอีเมลจะขึ้น `⚠️ (LINE ส่งไม่ออก)`**
- 🪤 **กับดักชื่อไฟล์ POS:** ชื่อไฟล์ PDF = *วันที่พิมพ์* ไม่ใช่วันของเนื้อหา → ส่งรายงานย้อนหลังหลายวันในวันเดียว ไฟล์จะลงท้ายเลขเดียวกันหมด แล้ว `importPosReports`/`backfillPos` ที่ใช้เลขท้ายเป็นคีย์จะทับกัน **วันหนึ่งหายเงียบ** (เคสจริง: 11 กับ 12 ส.ค. ถูกส่งซ้ำวันที่ 13 → วันที่ 11 หายไปจาก dashboard) → กู้ด้วย `importPosByDate()` ซึ่งเลือกอีเมลจาก **วันที่ในหัวเรื่อง**

**ฝั่งบอท (`app.py`) แก้ไป 6 commit:** บิลซื้อเด้งสรุปหนี้อัตโนมัติ · แก้ตัดบิลผิดใบ (ยอดตรงเป๊ะชนะวันที่บนสลิป) · ฟอร์มบล็อกยอดค้าง (ปี 2 หลัก `/26`→2026 ที่เดิมกลายเป็น 1983, วงเล็บหลายเลข, ไม่ข้ามบรรทัดเงียบ) · ลบบิล/จ่ายคืนยอดที่จับคู่ไว้ · `จัดยอดใหม่` ใช้กติกาเดียวกับตอนบันทึก · **แก้ `_cleanup_old_data` ที่พังทุกวันบน Postgres** (psycopg2 ตีความ `%` ใน `LIKE 'sent:%'` เป็น format spec → transaction rollback → **การลบข้อมูลเก่าไม่เคยทำงานเลยตั้งแต่ย้ายมา Postgres**)

~~**ยังค้าง:** UptimeRobot ยังจี้ URL เก่า · Supabase ของ E&M ควร export เก็บ · ยอด 25 ก.ค. / 29 ก.ค. ยังขาด~~
> ✅ **ปิดครบแล้ว 18 ส.ค.** (UptimeRobot ชี้ URL ใหม่ · export เก็บแล้ว · 25 ก.ค. มีข้อมูลอยู่แล้ว / 29-30 ก.ค. ร้านหยุด)
> — ดูโน้ตวันที่ 18 ส.ค. ด้านบนสุดของไฟล์นี้ **อย่าทำตามลิสต์นี้ซ้ำ**
...
~~**ยังค้าง:** ควรลบ endpoint `/api/db_import` + `/api/db_migrate` (เขียน/ทำลายได้)~~
> ✅ **ลบไปแล้ว** (ยืนยันในโค้ด 18 ส.ค. — เหลือ route ที่มี token 3 ตัว: `/api/slip_daily`, `/api/db_export` อ่านอย่างเดียว, `/api/push_owner` ส่งข้อความ ไม่เขียน DB)
